package com.myBookstoreProject.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private static final String[] PUBLIC_MATCHERS = { "/css/**", "/js/**", "/images/**", "/", "/forgetPassword",
			"/newUser", "/login", "/bookshelf", "/bookDetail/**", "/bookDetail", "/hours", "/faq", "/searchByCategory",
			"/search"};
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()./* antMatchers("/**") */antMatchers(PUBLIC_MATCHERS).permitAll().anyRequest()
				.authenticated();
		// login?error -> error è un parametro di login e può essere interrogato
