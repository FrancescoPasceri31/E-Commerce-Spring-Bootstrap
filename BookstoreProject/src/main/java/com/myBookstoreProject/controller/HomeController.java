...
import com.myBookstoreProject.service.impl.UserSecurityService;
import com.myBookstoreProject.utility.MailConstructor;
import com.myBookstoreProject.utility.SecurityUtility;
import com.myBookstoreProject.utility.USConstants;
...
public class HomeController {
	...
	private boolean fromCreditCardUpdate; // per aggiornare e non creare nuovo utente
	...
	public String addNewCreditCardPost(@ModelAttribute("userPayment") UserPayment userPayment,
		...
		if (fromCreditCardUpdate) {
			userPaymentService.removeById(userPayment.getId());
		}
		User user = userService.findByUsername(principal.getName());
		userService.updateUserBilling(userBilling, userPayment, user);

		fromCreditCardUpdate = false;
	...
	public String updateCreditCard(@ModelAttribute("id") Long creditCardId, Principal principal, Model model) {
		...
		} // only logged user who is owner of payment card can access
		else {
			model.addAttribute("user", user);
...
//		model.addAttribute("listOfShippingAddresses", true);
	...
	public String setDefaultShippingAddress(@ModelAttribute("defaultShippingAddressId") Long defaultShippingId,
		...
		User user = userService.findByUsername(principal.getName());
		userService.setUserDefaultShippingAddress(defaultShippingId, user);
	...
	public String removeUserShipping(@ModelAttribute("id") Long userShippingId, Principal principal, Model model) {
		...
		if (user.getId() != userShipping.getUser().getId()) {
			return "badRequestPage";
		} // consente di modificare solo a chi ha effettuato l'accesso
		else {
			model.addAttribute("user", user);
	...
	public String newUserPost(Locale locale, HttpServletRequest request, @ModelAttribute("email") String userEmail,
...
//		model.addAttribute("orderList", user.getOrderList());
	...
	public String updateUserInfo(@ModelAttribute("user") User user, @ModelAttribute("newPassword") String newPassword,
		...
		System.out.println("SAVED: "+currentUser.getEmail()+" -> TO SAVE: "+user.getEmail());
